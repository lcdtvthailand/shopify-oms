...
import { OrderStatus, getOrderStatusDisplay } from '@/lib/services/order-status'
        ...
        <div className="ml-3 flex-1">
          ...
          <div className="mt-3 text-sm">
            <div className="space-y-1">
              ...
              {status.cancelledAt && (
                <p className="text-gray-600">
                  <span className="font-medium">วันที่ยกเลิก:</span> {new Date(status.cancelledAt).toLocaleDateString('th-TH')}
                </p>
              )}
              {status.refundedAt && (
                <p className="text-gray-600">
                  <span className="font-medium">วันที่คืนเงิน:</span> {new Date(status.refundedAt).toLocaleDateString('th-TH')}
                </p>
              )}
            </div>
          </div>
